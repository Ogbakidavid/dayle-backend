...
export class WithdrawalRetryProcessor extends WorkerHost {
  ...
  async process(job: Job<any, any, string>): Promise<any> {
    ...
    this.logger.log(`Processing withdrawal retry for vault ${vaultId}, attempt ${retryCount + 1}`);
    ...
    try {
      ...
      if (!vault || vault.status !== VaultStatus.WITHDRAWAL_PENDING) {
        this.logger.warn(`Vault ${vaultId} not in WITHDRAWAL_PENDING state. Skipping retry.`);
        return;
      }

      await this.vaultsService.initiateWithdrawal(vaultId, userId, bankDetails, retryCount);
      this.logger.log(`Withdrawal retry successful for vault ${vaultId}`);
    } catch (error) {
      this.logger.error(`Withdrawal retry failed for vault ${vaultId}: ${error.message}`);
      throw error;
    }
  }
